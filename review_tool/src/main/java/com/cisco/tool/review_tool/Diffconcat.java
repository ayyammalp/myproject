...
public class Diffconcat {
	...
	public static void main(String[] args) {
		Diffconcat dObj = new Diffconcat();
...
//		KeyExist keyObj = new KeyExist();
		...
		try {
			...
			// Read json
			BufferedReader br = new BufferedReader(new FileReader("C:\\Users\\aypandia\\workspace\\cisco\\review_tool\\src\\main\\java\\com\\cisco\\tool\\review_tool\\input.json"));
			while ((fileLine = br.readLine()) != null) {
				json += fileLine + "\n";
			}
			Map<String, String> keybundle = new HashMap<String, String>();
			BufferedReader keybr = new BufferedReader(new FileReader("C:\\Users\\aypandia\\workspace\\cisco\\review_tool\\src\\main\\java\\com\\cisco\\tool\\review_tool\\key-module.json"));
			while ((fileLine = keybr.readLine()) != null) {
				keybundle.put(StringUtils.substringBefore(fileLine, ":").replaceAll("\"", ""), StringUtils.substringAfter(fileLine, ":"));
			}
										...
										for (int ii = 0; ii < jsonReadArr.length(); ii++) {
											...
											while (keys.hasNext()) {
												...
												for (int kk = 0; kk < innerArrCond.length(); kk++) {
													...
													if (jsonCond.has("i18key")) {
														...
														if (msg != null) {
															if(StringUtils.contains(msg, keyPattern)) {
																String multiMsg = StringUtils.substringAfter(msg, keyPattern);
																String message = keyPattern+multiMsg;
																while(checkKeyContains(multiMsg,keyPattern)) {
																	message = keyPattern+StringUtils.substringAfter(multiMsg, keyPattern);
															
																	resultSet.add(getCommaFilter(StringUtils.substringBefore(message, "+")));
																	
																	addsets.add(getCommaFilter(StringUtils.substringBefore(message, "+")));
																	
																	multiMsg=StringUtils.substringAfter(multiMsg, keyPattern);
																}
													...
													// Bundle syntax check
//												if (fileName.endsWith("i18nMessageBundle."+jsonRead) || fileName.contains("i18nMessageBundle."+jsonRead)) {
//
//														if (jsonCond.has("contains")) {
//															contains = jsonCond.get("contains").toString();
//															msg = dObj.checkBundleSyntax(fileName,
//																	lineNo + " " + add.trim(), contains,
//																	"containsCheck");
//															resultSet.add(msg);
//														}
//														if (jsonCond.has("endsWith")) {
//															ends = jsonCond.get("endsWith").toString();
//															msg = dObj.checkBundleSyntax(fileName,
//																	lineNo + " " + add.trim(), ends, "endsCheck");
//															resultSet.add(msg);
//														}
//
//														// Retrieve key from bundle
//														if (add.contains(contains)) {
//															String split[] = add.split(contains);
//															dObj.addFileToMap(fileName, split[0].toString().trim(),
//																	bundleMap);
//														}
//													} else {
//														if (add.contains(jsonRead)) {
//															if (jsonCond.has("file")) {
//																file = jsonCond.getString("file");
//															}
//															if (jsonCond.has("equals")) {
//																equals = jsonCond.getString("equals");
//															}
//															if (jsonCond.has("regex")) {
//																regex = jsonCond.getString("regex");
//
//																// Retrieve key from technical file
//																if (file != "") {
//																	retrieveKey = regObj.getKeyFromLine(add, regex,
//																			file, equals);
//																} else {
//																	retrieveKey = regObj.getKeyFromLine(add, regex,
//																			"No", equals);
//																}
//																dObj.addFileToMap(fileName, retrieveKey, keyMap);
//															}
//															if (jsonCond.has("suggestion")) {
//																suggestion = jsonCond.getString("suggestion");
//															}
//															if (fileName.endsWith(".html")) {
//																if (!add.contains(equals)) {
//																	msg = "<font color=\"red\">WARNING</font> check the format is present "
//																			+ equals + " } in line " + lineNo + " "
//																			+ add;
//																	resultSet.add(msg);
//																}
//															}
